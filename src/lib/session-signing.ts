...
export async function verifySessionToken(signed: string): Promise<string | null> {
  ...
  // NOTE: DB-backed session revocation CANNOT live here — this file is
  // imported by Edge middleware, where Prisma/Node modules are unavailable.
  // The revocation check (revoked_sessions, expiresAt-bounded) runs in the
  // Node-runtime callers instead: requireAuth() in lib/api-helpers.ts and
